...
const VideoPlayerPage = () => {
  ...
  const handleLike = async () => {
    ...
    try {
      if (liked) {
        ...
        // Primero, verificamos si ya pagamos por este like
        const { data: pointsData } = await supabase
          .from('points_transactions')
          .select('id')
          .eq('user_id', user.id)
          .eq('reference_id', videoId)
          .eq('transaction_type', 'give_like') // Asumiendo que el RPC usa 'give_like'
          .limit(1);

        // Si NO hay registro de pago, intentamos pagar.
        if (!pointsData || pointsData.length === 0) {
          setHasEarnedLikePoints(true); // Marcamos como intentado
          ...
          // Si ya había un registro de pago, solo marcamos el estado y no llamamos al servicio.
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      ...
      // 🛑 LÍNEA ELIMINADA: RPC de incremento de comentarios
      setVideoCounters(prev => ({
        ...prev,
      ...
      // ✅ VERIFICACIÓN DE PUNTOS
      // Primero, verificamos si ya pagamos por este comentario
      const { data: pointsData } = await supabase
        .from('points_transactions')
        .select('id')
        .eq('user_id', user.id)
        .eq('reference_id', videoId)
        .eq('transaction_type', 'comment') // Asumiendo que el RPC usa 'comment'
        .limit(1);
                                      ...
                                      {comment.replies.map((reply) => (
                                        <div key={reply.id} className="flex gap-2 md:gap-3 ml-4 md:ml-6">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs md:text-sm text-foreground mt-1">
                                              {reply.content}
                                            </Por supuesto. Comprendo tu frustración. El problema que describes es un **fallo de sincronización y de lógica de base de datos** muy complejo.

Aquí está el análisis final de los tres problemas y el plan de acción definitivo para solucionarlos, asumiendo que el código de tu *frontend* (`index (15).jsx`) ya está en su versión final.

---

### 1. 🛑 Fallo de Recarga al Cambiar de Pestaña

El problema de que la página se actualice al cambiar de pestaña **NO** es un comportamiento normal del navegador, sino un síntoma de un **error crítico en el *frontend***.

* **Causa:** Tu consola muestra el error: `Error al cargar videos relacionados: ... "column videos.likes_count does not exist"`.
* **Análisis:** Este error ocurre porque en tu archivo `index.jsx`, la función `loadRelatedVideos` (que se ejecuta al cargar la página) intenta seleccionar la columna `likes_count` de la tabla `videos`. Sin embargo, hemos eliminado esa columna porque era inconsistente y ahora usamos el conteo directo.
* **Impacto:** Este error "rompe" React. Cuando React se rompe y vuelves a enfocar la pestaña, el *framework* (o el servidor de desarrollo) fuerza una recarga completa para intentar recuperarse.

**Solución (Crítica):** Debemos eliminar la selección de las columnas `likes_count` y `comments_count` de la función `loadRelatedVideos` en `index.jsx`.

---

### 2. 🛑 Fallo del Contador de Vistas (No se suma)

* **Causa:** El contador de vistas no se suma por la misma razón que los puntos no se sumaban antes: el RPC `increment_video_views` está siendo llamado por un usuario (`Invoker`), pero no tiene permisos (`SECURITY DEFINER`) para actualizar la tabla `public.videos`.

**Solución (SQL):** Debemos aplicar `SECURITY DEFINER` a esa función.

---

### 3. 🛑 Fallo de "Farming" (Sigue dando puntos)

* **Causa:** Sigues ganando puntos al quitar y dar *Like* porque el *backend* (la DB) está fallando silenciosamente al registrar el primer pago.
* **Análisis (Consola):** La consola muestra el error `ERROR: 23514: ... violates check constraint "points_transactions_transaction_type_check"`.
* **Impacto:** Este error `23514` significa que la tabla `points_transactions` **rechaza** la inserción de puntos (porque el `transaction_type` es incorrecto, ej. `'give_like'` no está en la lista de valores permitidos). Como el pago nunca se registra, el `UNIQUE CONSTRAINT` (la restricción de *Farming*) nunca se activa.
* **Por qué el *Frontend* Miente:** El *frontend* muestra "¡ÉXITO!" porque el RPC `track_mission_progress` devuelve `success: true` **antes** de que el RPC `update_user_points` falle con el error `23514`.

**Solución (SQL):** Debemos corregir la función SQL `track_mission_progress` para que use un valor que la tabla acepte (como `'other'`).

---

## 🛠️ Plan de Corrección Definitivo (2 Pasos)

### Paso 1: Corregir el Código del *Frontend* (JavaScript)

Te proporciono el archivo `src/pages/VideoPlayerPage/index.jsx` completo. Este archivo incluye:
1.  **Corrección del Crash (Recarga):** La función `loadRelatedVideos` ya no selecciona las columnas eliminadas (`likes_count`, `comments_count`).
2.  **Corrección del Farming (Lógica):** La función `handleLike` está verificada para no borrar el historial de `points_transactions`.

### Paso 2: Corregir las Funciones de Base de Datos (SQL)

Te proporciono los comandos SQL para:
1.  **Corregir el Contador de Vistas:** Aplicar `SECURITY DEFINER` a `increment_video_views`.
2.  **Corregir el *Farming* (Error 23514):** Actualizar `track_mission_progress` para que use el valor `'other'`.

---

## 📁 `src/pages/VideoPlayerPage/index.jsx` (Completo y Corregido)

Este código corrige el *crash* que causa la recarga de la pestaña.
